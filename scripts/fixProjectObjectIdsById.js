// Usage: node scripts/fixProjectObjectIdsById.js
...
async function run() {
  ...
  try {
    ...
    // Fix members[0].user and createdBy
    const fixedMembers = (project.members || []).map(m => {
      if (m && typeof m.user === 'string') {
        return { ...m, user: new ObjectId(m.user) };
      }
      return m;
    });
    const fixedCreatedBy = typeof project.createdBy === 'string' ? new ObjectId(project.createdBy) : project.createdBy;
